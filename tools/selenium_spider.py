...
"""
基于selenium版本进一步封装 只针对于谷歌浏览器 其他浏览器需要自己封装
"""
import json
...
class SeleniumSpider(WebDriver):
    ...
    def cookies_dict_to_selenium_cookies(self, cookies: dict, domain):
        ...
        for key, value in cookies.items():
            # requests 有bug 域区分的不是很清楚 手动区分 只限全国电信口能用
            temp_cookies.append({"name": key, "value": value, "domain": domain})
        return temp_cookies

    def get_(self, url: str, cookies=None, domain=None):
        """
        请求数据
        :param url: 待请求的url
    ...
    def web_driver_wait_ruishu(self, time: int, rule: str, num: str):
        ...
        while time:
            ...
            try:
                html = Selector(text=response["value"])
